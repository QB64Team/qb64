DECLARE FUNCTION ffnran% (x AS INTEGER)
DECLARE SUB play2 (a$)
...
DIM SHARED SPEEDCONST AS INTEGER
DIM SHARED TRUE AS INTEGER
DIM SHARED FALSE AS INTEGER
DIM SHARED HITSELF AS INTEGER
DIM SHARED BACKATTR AS INTEGER
DIM SHARED OBJECTCOLOR AS INTEGER
DIM SHARED WINDOWCOLOR AS INTEGER
DIM SHARED SUNATTR AS INTEGER
DIM SHARED SUNHAPPY AS INTEGER
DIM SHARED SUNSHOCK AS INTEGER
DIM SHARED RIGHTUP AS INTEGER
DIM SHARED LEFTUP AS INTEGER
DIM SHARED ARMSDOWN AS INTEGER
...
'LOCATE 7, 1
' PRINT x; y
'x is 320 because of a scope problem, it is referencing the incorrect variable
...
FUNCTION ffnran (x AS INTEGER)
'DEF FnRan (x) = INT(RND(1) * x) + 1
ffnran = INT(RND(1) * x) + 1
END FUNCTION
...
IF UCASE$(Char$) = "V" THEN
    ...
    play2 "t120o1l16b9n0baan0bn0bn0baaan0b9n0baan0b"
    '    Rest .3

    PUT (x - 13, y), GorR&(), PSET
    PUT (x + 47, y), GorL&(), PSET
    play2 "o2l16e-9n0e-d-d-n0e-n0e-n0e-d-d-d-n0e-9n0e-d-d-n0e-"
    '    Rest .3

    PUT (x - 13, y), GorL&(), PSET
    PUT (x + 47, y), GorR&(), PSET
    play2 "o2l16g-9n0g-een0g-n0g-n0g-eeen0g-9n0g-een0g-"
    '    Rest .3

    PUT (x - 13, y), GorR&(), PSET
    PUT (x + 47, y), GorL&(), PSET
    play2 "o2l16b9n0baan0g-n0g-n0g-eeen0o1b9n0baan0b"
    '    Rest .3

    FOR i = 1 TO 4
        PUT (x - 13, y), GorL&(), PSET
        PUT (x + 47, y), GorR&(), PSET
        play2 "T160O0L32EFGEFDC"
        '      Rest .1
        PUT (x - 13, y), GorR&(), PSET
        PUT (x + 47, y), GorL&(), PSET
        play2 "T160O0L32EFGEFDC"
...
DO
    ...
    DO
        FOR i = BHeight - 3 TO 7 STEP -WDifV
            IF Mode <> 9 THEN
                WinColr = (ffnran(2) - 2) * -3
            ELSEIF ffnran(4) = 1 THEN
                WinColr = 8
            ELSE
                WinColr = WINDOWCOLOR
            END IF
...
FOR i = 1 TO 2
    IF i = 1 THEN BNum = ffnran(2) + 1 ELSE BNum = LastBuilding - ffnran(2)
...
FOR i = 1 TO NumGames
    ...
    'LOCATE 5, 1
    ' PRINT GorillaX(1); GorillaY(1), GorillaX(2); GorillaY(2)
    ...
    DO WHILE Hit = FALSE
        ...
        'LOCATE 6, 1
        'PRINT GorillaX(1); GorillaY(1), GorillaX(2); GorillaY(2), tosser
        'theory, because this is a function the code isn't quite correct!
...
FOR i = 1 TO ticks
    DO: newtime! = TIMER: LOOP WHILE newtime! = oldtime!
    oldtime! = newtime!
NEXT
