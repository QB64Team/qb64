...
//Overload to use memory & pointers not file handles
static int ReadFromMemory_size;
int fread(char *dest, int ignore, int bytes, char **fp ){
	if (bytes>ReadFromMemory_size) bytes=ReadFromMemory_size;
	ReadFromMemory_size-=bytes;
	memmove(dest,*fp,bytes);
	*fp+=bytes;
	return bytes;
}
bool SafeFread (char *dest, int bytes, int ignore, char **fp){
	if (bytes>ReadFromMemory_size) return false;
	ReadFromMemory_size-=bytes;
	memmove(dest,*fp,bytes);
	*fp+=bytes;
	return true;
...
bool BMP::ReadFromMemory( const char* memory, int size )
{ 

ReadFromMemory_size=size;
