...
#ifdef QB64_WINDOWS
    #include "..\\temp3\\regsf.txt"
    #include "..\\temp3\\global.txt"
    #else
    #include "../temp3/regsf.txt"
    #include "../temp3/global.txt"
#endif
...
void set_dynamic_info(){
    #ifdef QB64_WINDOWS
        #include "..\\temp3\\dyninfo.txt"
        #else
        #include "../temp3/dyninfo.txt"
    #endif
}
...
void sub_clear(int32 ignore,int32 ignore2, int32 stack,int32 passed){
    ...
    #ifdef QB64_WINDOWS
        #include "..\\temp3\\clear.txt"
        #else
        #include "../temp3/clear.txt"
    #endif
    ...
    if ((func_command(0, 0))->len>= 32 ){
        if (qbs_equal(qbs_right(func_command(0, 0), 4 ),qbs_new_txt_len(".tmp",4))){
            if (qbs_equal(func_mid(func_command(0, 0),(func_command(0, 0))->len- 31 , 25 ,1),qbs_new_txt_len("(unique-tag:=/@*$+-)chain",25))){
                ...
                //read COMMON data
                #ifdef QB64_WINDOWS
                    #include "..\\temp3\\inpchain.txt"
                    #else
                    #include "../temp3/inpchain.txt"
                #endif
...
void sub_chain(qbs* f){
    if (new_error) return;
        ...
        #ifdef QB64_WINDOWS
            #include "..\\temp3\\chain.txt"
            #else
            #include "../temp3/chain.txt"
        #endif
                            ...
                            if (onstrig[(di<<8)+i].active==1){//if STOPped, event will be postponed
                                if (onstrig[(di<<8)+i].state){
                                    if (!onstrig_inprogress){
                                        ...
                                        switch(x){
                                            #ifdef QB64_WINDOWS
                                                #include "..\\temp3\\onstrig.txt"
                                                #else
                                                #include "../temp3/onstrig.txt"
                                            #endif
                                            //example.....
                                            //case 1:
                                            //...
                ...
                if (onkey[i].active==1){//if STOPped, event will be postponed
                    if (onkey[i].state){
                        if (!onkey_inprogress){
                            ...
                            switch(x){
                                #ifdef QB64_WINDOWS
                                    #include "..\\temp3\\onkey.txt"
                                    #else
                                    #include "../temp3/onkey.txt"
                                #endif
                                //example.....
                                //case 1:
                                //...
                    ...
                    if (ontimer[i].active==1){//if timer STOPped, event will be postponed
                        if (ontimer[i].state==1){
                            ...
                            switch(x){
                                ...
                                #ifdef QB64_WINDOWS
                                    #include "..\\temp3\\ontimer.txt"
                                    #else
                                    #include "../temp3/ontimer.txt"
                                #endif
                                //example.....
                                //case 1:
                                //...
...
void QBMAIN(void *unused)
{
    ...
    */
    ...
    #ifdef QB64_WINDOWS
        #include "..\\temp3\\maindata.txt"
        #include "..\\temp3\\runline.txt"
        #include "..\\temp3\\mainerr.txt"
        if (timer_event_occurred){
            timer_event_occurred--;
            #include "..\\temp3\\ontimerj.txt"
        }
        if (key_event_occurred){
            key_event_occurred--;
            #include "..\\temp3\\onkeyj.txt"
        }
        if (strig_event_occurred){
            strig_event_occurred--;
            #include "..\\temp3\\onstrigj.txt"
        }
        chain_input();
        #include "..\\temp3\\main.txt"
    #else
        #include "../temp3/maindata.txt"
        #include "../temp3/runline.txt"
        #include "../temp3/mainerr.txt"
        if (timer_event_occurred){
            timer_event_occurred--;
            #include "../temp3/ontimerj.txt"
        }
        if (key_event_occurred){
            key_event_occurred--;
            #include "../temp3/onkeyj.txt"
        }
        if (strig_event_occurred){
            strig_event_occurred--;
            #include "../temp3/onstrigj.txt"
        }
        chain_input();
        #include "../temp3/main.txt"
    #endif
